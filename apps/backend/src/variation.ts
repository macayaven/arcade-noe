...
const VariationSchema = z.object({
  ...
  // Visual theme
  theme: z.object({
    name: z.enum(['neon', 'retro', 'pastel', 'dark', 'ocean', 'sunset', 'forest', 'cosmic', 'fire', 'ice', 'toxic', 'cyber']),
  ...
  // Game mechanics
  difficulty: z.object({
    level: z.enum(['easy', 'normal', 'hard', 'extreme']),
    speedMultiplier: z.number(),
    complexityBonus: z.number(),
  }),
...
export function generateVariation(gameId: string): Variation {
  ...
  // Select theme
  const themeNames = Object.keys(COLOR_THEMES) as (keyof typeof COLOR_THEMES)[];
  ...
  const selectedModifiers: typeof allModifiers[number][] = [];
...
function generateGameSpecificParams(gameId: string, rng: () => number): Record<string, any> {
  switch (gameId) {
    case 'snake':
      return {
